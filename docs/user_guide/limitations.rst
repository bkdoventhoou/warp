...
* ``wp.config.verify_launch_array_access`` only fully verifies cross-device
  :class:`wp.array <warp.array>` arguments when Warp can determine the
  allocation kind. Arrays backed by custom or externally wrapped allocators fail
  closed in verification mode, and directly passed ``__array_interface__`` or
